...
public class LinLogLayout {
	 ...
	 * @return read graph.
	 */
	private Map<String,Map<String,Double>> readGraph(String filename) {
	 ...
	 */
	private Map<String,Map<String,Double>> makeSymmetricGraph
	 ...
	 */
	private Map<String,Node> makeNodes(Map<String,Map<String,Double>> graph) {
     ...
     */
    private List<Edge> makeEdges(Map<String,Map<String,Double>> graph, 
	 ...
	 */
	private Map<Node,double[]> makeInitialPositions(List<Node> nodes, boolean is3d) {
		...
		try {
			...
			for (Node node : nodeToPosition.keySet()) {
                ...
                */
                ...
                clusterMap.put(node.name + "\t" + String.format("%.4f", node.weight), cluster);
                
			}
			
			// file.close();
			
			FileFunction.writeMapStrInt(clusterMap, filename);
	...
	public void run(String dim, String input, String output) {
		/*
		...
		//String dim = "2";
		//String input = "D:\\Development\\WikiCluster\\all\\IT과학 IT일반.txt";
		//String output = "D:\\Development\\WikiCluster\\all\\IT과학 IT일반 output.txt";
		
		Map<String,Map<String,Double>> graph = readGraph(input);
		...
		Map<Node,double[]> nodeToPosition = makeInitialPositions(nodes, dim.equals("3"));
		...
		writePositions(nodeToPosition, nodeToCluster, output);
